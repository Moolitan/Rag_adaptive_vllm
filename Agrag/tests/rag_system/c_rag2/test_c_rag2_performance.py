...
RESULTS_DIR = ROOT / "tests" / "results" / "crag2_bug"
...
def run_single_test(question: str, retrieval_k: int = 15) -> PerformanceResult:
    """运行单个 CRag2 查询并收集性能数据"""
    clear_performance_records()
    ...
    return PerformanceResult(
        ...
        web_search_rounds=0,  # CRag2 不跟踪轮次
...
def run_benchmark(
    ...
    for i, q in enumerate(questions):
        ...
        perf = run_single_test(q, retrieval_k=retrieval_k)
        results.append(perf)
        ...
        if verbose:
            print(
                ...
                f"WebSearch: {perf.used_web_search}"
            )
...
def main():
    ap = argparse.ArgumentParser(description="CRag2 Performance Test (FAISS + DataCollector)")
    ...
    save_workflow_graph(RESULTS_DIR)

    questions = questions[:min(args.limit, len(questions))]

    print("=" * 60)
    print("CRag2 Performance Test (FAISS)")
    print("=" * 60)
    ...
    monitor.start()
    try:
        results = run_benchmark(questions, retrieval_k=args.retrieval_k, verbose=args.verbose)
