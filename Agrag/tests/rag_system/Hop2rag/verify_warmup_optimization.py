...
from Rag.hop2_rag_performancy_concurrent import (
    get_embedding_model,
...
def test_singleton():
    ...
    print(f"   Second call: {time2:.4f}s (cached, {time1/time2:.0f}x faster)")
    print(f"   Third call: {time3:.4f}s (cached, {time1/time3:.0f}x faster)")


def test_retriever_cache():
    """测试 Retriever 缓存"""
    if not PERSIST_DIR:
        print("\n⚠️  SKIP: AGRAG_PERSIST_DIR not set")
        return
    ...
    print("\n[1] First call to get_custom_retriever(k=10)...")
    start = time.time()
    retriever1 = get_custom_retriever(PERSIST_DIR, COLLECTION_NAME, k=10)
    time1 = time.time() - start
    print(f"    Time: {time1:.2f}s")
    print(f"    Retriever ID: {id(retriever1)}")

    print("\n[2] Second call to get_custom_retriever(k=10)...")
    start = time.time()
    retriever2 = get_custom_retriever(PERSIST_DIR, COLLECTION_NAME, k=10)
    time2 = time.time() - start
    print(f"    Time: {time2:.2f}s")
    print(f"    Retriever ID: {id(retriever2)}")

    print("\n[3] Call with different k (k=20)...")
    start = time.time()
    retriever3 = get_custom_retriever(PERSIST_DIR, COLLECTION_NAME, k=20)
    time3 = time.time() - start
    print(f"    Time: {time3:.2f}s")
    print(f"    Retriever ID: {id(retriever3)}")

    # 验证缓存
    assert retriever1 is retriever2, "Same k should return cached retriever!"
    assert retriever1 is not retriever3, "Different k should return different retriever!"
    print("\n✅ PASS: Retriever cache works correctly")
    print(f"   First call (k=10): {time1:.2f}s (creating)")
    print(f"   Second call (k=10): {time2:.4f}s (cached, {time1/time2:.0f}x faster)")
    print(f"   Third call (k=20): {time3:.2f}s (creating new)")
...
def test_warmup():
    ...
    start = time.time()
    warmup_resources(PERSIST_DIR, COLLECTION_NAME, k=10)
    ...
    print(f"\n[3] After warmup, calling get_custom_retriever()...")
    start = time.time()
    _ = get_custom_retriever(PERSIST_DIR, COLLECTION_NAME, k=10)
    time2 = time.time() - start
    ...
    print(f"   Post-warmup get_custom_retriever(): {time2:.4f}s")
