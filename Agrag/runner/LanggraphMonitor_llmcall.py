...
class DataCollector(BaseCallbackHandler):
    ...
    def __init__(self, debug: bool = False, track_prompts: bool = True, encoding_name: str = "cl100k_base"):
        ...
        # run_id -> llm_calls 索引的映射（用于在 on_llm_end 中找到对应的记录）
        self.llm_run_to_index: Dict[str, int] = {}
    ...
    def _extract_response_text(self, response: LLMResult) -> Optional[str]:
        """从 LLMResult 中提取回答文本"""
        if not response or not response.generations:
            return None
        
        try:
            # LLMResult.generations 是一个二维列表：List[List[Generation]]
            # 通常我们只关心第一个输入的第一个生成结果
            all_texts = []
            for gen_list in response.generations:
                for gen in gen_list:
                    if hasattr(gen, 'text') and gen.text:
                        all_texts.append(gen.text)
                    elif hasattr(gen, 'message') and hasattr(gen.message, 'content'):
                        # 对于 ChatGeneration，需要从 message 中获取内容
            ...
            if len(full_response) > 10000:
                return full_response[:10000] + "...[truncated]"
            return full_response
        except Exception as e:
            if self.debug:
                print(f"[DEBUG] Failed to extract response text: {e}")
            return None
    ...
    def on_llm_start(self, serialized: Dict[str, Any], prompts: List[str], **kwargs: Any) -> None:
        ...
        if self.track_prompts and prompts:
            ...
            llm_call_info = {
                ...
                "response": None,  # 将在 on_llm_end 中填充
                "response_tokens": 0,  # 将在 on_llm_end 中填充
                "timestamp": time.time(),
            }
            # 记录 run_id 到 llm_calls 索引的映射
            self.llm_run_to_index[run_id] = len(self.llm_calls)
            self.llm_calls.append(llm_call_info)

            if self.debug:
                print(f"[DEBUG] llm_call_start: node={llm_call_info['node_name']}, prompt_tokens={prompt_tokens}")
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        # 提取 LLM 的回答并更新 llm_calls 记录
        if self.track_prompts and run_id in self.llm_run_to_index:
            llm_call_index = self.llm_run_to_index[run_id]
            if llm_call_index < len(self.llm_calls):
                # 从 LLMResult 中提取回答文本
                response_text = self._extract_response_text(response)
                response_tokens = self._count_tokens(response_text) if response_text else 0
                
                # 更新记录
                self.llm_calls[llm_call_index]["response"] = response_text
                self.llm_calls[llm_call_index]["response_tokens"] = response_tokens
                
                if self.debug:
                    node_name = self.llm_calls[llm_call_index].get("node_name")
                    print(f"[DEBUG] llm_call_end: node={node_name}, response_tokens={response_tokens}")
            
            # 清理映射
            del self.llm_run_to_index[run_id]
